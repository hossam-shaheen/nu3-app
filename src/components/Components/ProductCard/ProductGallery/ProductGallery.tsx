...
export const ProductGallery: FunctionComponent<{
  mainImage: ProductAttributeProps["productImage"] | undefined;
  thumbnails: ProductAttributeProps["productImages"] | undefined;
}> = ({ mainImage, thumbnails }): JSX.Element => {
  ...
  return (
    <div className={classes["product-gallery"]}>
      <div className={classes["product-gallery-main"]}>
        {currentMainImage && (
          <img src={currentMainImage?.src} alt={currentMainImage?.alt} />
        )}
      </div>
        ...
        {thumbnails?.map((image: { id: number; src: string; alt: string }) => {
          return (
            <li
              key={image.id}
              className={
                currentMainImage?.id === image.id ? classes["active"] : ""
              }
              onClick={onChangeThumbnails.bind(
                null,
                image.id,
                image.src,
                image.alt
